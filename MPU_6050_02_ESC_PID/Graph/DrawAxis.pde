void drawIt() 
{
  for(int i=0; i<25; i++)
  {
    ...
    beginShape();
    vertex(0, displayBuffer[i][0]);
    
    for (int j = 1; j < displayBuffer[i].length; j++) 
    {
      if (  (displayBuffer[i][j] < height/4 && displayBuffer[i][j - 1] > height/4*3) || 
            (displayBuffer[i][j] > height/4*3 && displayBuffer[i][j - 1] < height/4)) 
      {
        endShape();
        beginShape();
      }
      vertex(i, displayBuffer[i][j]);
    }
    endShape();  
    ...
    for (int j = 1; j < displayBuffer[i].length; j++)
    {
      displayBuffer[i][j-1] = displayBuffer[i][j];
    }
